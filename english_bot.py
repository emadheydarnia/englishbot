...
GERMAN_AI_TOPICS = {
    "de_grammar": {
        ...
        "prompt": """You are a friendly English teacher at Emad Eng Lab for German-speaking students.
IMPORTANT: Speak ONLY in German. Never use Persian or Farsi.
Personality: warm, encouraging, occasionally funny — like a good friend who happens to know English well.

Teaching method:
- Give German sentences one at a time that the student must translate to English
- Cover grammar topics: Present Simple, Present Continuous, Past Simple, Present Perfect, Modals, Conditionals, Passive Voice, Relative Clauses
- After each answer give feedback in German:
  ✅ Correct English translation
  📝 Grammar point explained briefly in German
  💡 One more example sentence in German to translate
- 20 sentences per session, gradually increasing difficulty (A2 to B2)
- German sentences must be natural and everyday
- After 20 sentences give a brief report in German

Language: ALL explanations, feedback, encouragement in GERMAN
...
def german_ielts_menu_keyboard():
    ...
    for k, v in IELTS_AI_TOPICS.items():
        rows.append([InlineKeyboardButton(v["label"], callback_data="ai_topic_de_ielts_" + k)])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        if topic_key.startswith("de_"):
            topic = GERMAN_AI_TOPICS.get(topic_key)
        elif topic_key.startswith("de_ielts_"):
            # IELTS برای آلمانی — همون prompt ولی زبان آلمانی
            ielts_key = topic_key[9:]  # حذف "de_ielts_"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in reply_kb_map:
        # اگه exam یا ثبت‌نام فعاله، نادیده بگیر
        if state.get("active") or state.get("step"):
            pass  # ادامه بده تا جواب exam پردازش بشه
        else:
            clear_ai_session(user_id)
            await update.message.reply_text("لطفاً یه گزینه انتخاب کن:", reply_markup=main_menu_keyboard())
            return
