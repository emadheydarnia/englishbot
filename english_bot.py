...
def a2z_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📚 IELTS Vocabulary", callback_data="cat_ielts")],
...
def ielts_menu_keyboard():
    ...
    try:
        from ielts_exam_data import IELTS_CATEGORIES
        for k, v in sorted(IELTS_CATEGORIES.items()):
            rows.append([InlineKeyboardButton(v["label"], callback_data="ielts_cat_" + k)])
    except:
        pass
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        await query.edit_message_text("📚 IELTS Vocabulary\n\nیه فصل انتخاب کن:", reply_markup=ielts_menu_keyboard())

    elif data.startswith("ielts_cat_"):
        cat_key = data[10:]
        cat = IELTS_CATEGORIES.get(cat_key)
        if not cat:
            await query.edit_message_text("فصل پیدا نشد.", reply_markup=main_menu_keyboard())
            return
        rows = [[InlineKeyboardButton(f"📝 Exam {i}", callback_data="exam_" + ek)] for i, ek in enumerate(cat["exams"], 1)]
        rows.append([InlineKeyboardButton("🔙 Back", callback_data="cat_ielts")])
        await query.edit_message_text(
            f"📚 {cat['label']}\n\nیه exam انتخاب کن:",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_kb_map = {
        "📚 ارشد-دکتری": "cat_arshad",
        "🔤 A2Z English": "cat_a2z",
        "🎯 Level Test": "ai_topic_level_test",
        "📝 Exams": "cat_exams",
        "🤖 AI Practice": "cat_ai",
    }
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        return
    msg = "📊 آمار همه دانش\u200cآموزان\n" + "━" * 30 + "\n"
    for r in rows:
        avg = str(r['avg_score']) + "%" if r['avg_score'] else "—"
        last = r['last_active'].strftime("%m/%d") if r['last_active'] else "—"
        msg += "👤 " + str(r["name"]) + " (کلاس " + str(r["student_class"]) + ") | " + str(r["activities"]) + " فعالیت | میانگین: " + avg + " | آخرین: " + last + "\n"
    await update.message.reply_text(msg)
