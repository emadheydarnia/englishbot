...
OPENROUTER_API_KEY = os.environ.get("GEMINI_API_KEY", "")  # OpenRouter API key
...
OPENROUTER_MODELS = [
    "google/gemini-2.5-flash-preview:free",      # Gemini 2.5 رایگان
    "google/gemini-2.0-flash-thinking-exp:free", # Gemini فکری رایگان
    "deepseek/deepseek-chat-v3-0324:free",       # DeepSeek رایگان — پشتیبان
]

def call_gemini_api(history, new_message, model=None):
    """Call OpenRouter API. Tries OPENROUTER_MODELS in order if no model specified."""
    messages = []
    for item in history:
    ...
    for m in models_to_try:
        ...
        headers = {
            ...
            "HTTP-Referer": "https://t.me/A2Z_English_bot",
            "X-Title": "A2Z English Bot"
        }
        try:
