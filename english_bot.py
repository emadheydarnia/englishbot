...
def speaking_keyboard():
    """منوی Interactive Speaking — دقیقاً همون grammar topics"""
    rows = []
    ...
    grammar_topics = [k for k in AI_TOPICS.keys() if k not in excluded]
    for key in grammar_topics:
        # هر grammar topic یه speaking version داره
        speaking_key = "conv_" + key
        label = AI_TOPICS[key]["label"].replace("📗","💬").replace("📘","💬").replace("📙","💬").replace("📕","💬").replace("📓","💬").replace("📒","💬").replace("📔","💬").replace("📃","💬").replace("🔄","💬").replace("📚","💬").replace("📖","💬").replace("🔀","💬").replace("⚙️","💬").replace("✏️","💬")
        if speaking_key in AI_TOPICS:
            rows.append([InlineKeyboardButton(label, callback_data="ai_topic_" + speaking_key)])
        else:
            # اگه speaking version نداشت، grammar topic رو با پرامپت speaking بده
...
def ielts_menu_keyboard():
    ...
    rows = []
    for k, v in IELTS_AI_TOPICS.items():
        ch_num = k.replace("ielts_ch", "")
        # دکمه Vocabulary + Conversation برای هر فصل
        rows.append([
            InlineKeyboardButton(v["label"], callback_data="ai_topic_" + k),
            InlineKeyboardButton("💬", callback_data="ai_topic_conv_" + k)
        ])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        await query.edit_message_text(
            "💬 Interactive Speaking\n\nیه موضوع گرامری انتخاب کن:\n\n🎭 AI نقش یه شخصیت رو بازی می‌کنه\n✍️ تو انگلیسی صحبت کن\n📊 آخرش feedback کامل میگیری",
...
Start the scene now — introduce your character and setting briefly, then begin."""
        ...
        topic = {{"label": f"💬 Speaking: {grammar_label.split(' ', 1)[-1] if ' ' in grammar_label else grammar_label}", "prompt": speaking_prompt}}
