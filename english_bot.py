...
# اضافه کردن IELTS و German topics به AI_TOPICS
...
def german_grammar_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📖 Grammar Practice", callback_data="ai_topic_de_grammar")],
        [InlineKeyboardButton("🎯 Level Test", callback_data="ai_topic_de_level_test")],
        [InlineKeyboardButton("🔙 Back", callback_data="lang_german")],
    ])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "back_main":
        ...
        # بررسی German topics — از GERMAN_AI_TOPICS جدا
        if topic_key.startswith("de_"):
            topic = GERMAN_AI_TOPICS.get(topic_key)
            if not topic:
                # اگه توی GERMAN_AI_TOPICS نبود، ممکنه level_test باشه
                topic = GERMAN_AI_TOPICS.get("de_" + topic_key)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in reply_kb_map:
        ...
        if state.get("active") or state.get("step") or ai_sess.get("active"):
            return  # وسط exam یا AI session — نادیده بگیر
        else:
            clear_ai_session(user_id)
